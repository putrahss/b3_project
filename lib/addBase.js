...
// ===== harga loader =====
function loadHarga() {
  const p = path.resolve(process.cwd(), 'andy', 'harga.json');
  if (!fs.existsSync(p)) throw new Error('File ./andy/harga.json tidak ditemukan.');
...
function loadVpsList() {
  const p = path.resolve(process.cwd(), 'andy', 'vps.json');
